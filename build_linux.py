...
class BuildThread(QThread):
    ...
    def run(self):
        try:
            self.log_update.emit("Starting Linux build...\n")
            ...
            self.log_update.emit("Building standalone Linux executable with PyInstaller...\n")
            ...
            if result.returncode == 0:
                executable_path = os.path.join(output_dir, "MoonshineBrowser")
                # Make executable
                os.chmod(executable_path, 0o755)
                self.log_update.emit("\n✓ Build successful!\n")
                self.log_update.emit(f"Executable created at: {executable_path}\n")
                self.log_update.emit("Run it with: ./dist/MoonshineBrowser\n")
...
class BuilderGUI(QWidget):
    ...
    def initUI(self):
        self.setWindowTitle('Moonshine Browser - Linux Installer Builder')
        self.setGeometry(100, 100, 600, 500)

        layout = QVBoxLayout()

        title = QLabel('Moonshine Browser Linux Installer Builder')
        layout.addWidget(title)

        info = QLabel('Click "Start Build" to create a standalone Linux executable')
